document.addEventListener("DOMContentLoaded", () => {
 ...
 //진입시 비밀번호 확인
  ...
  // 파일 업로드
  const shelterFileInput = document.getElementById("shelter-file-input");
  const shelterFileBtn = document.getElementById("shelter-file-btn");
  const shelterFileName = document.getElementById("shelter-file-name");
  ...
  if (shelterFileBtn && shelterFileInput) {
    shelterFileBtn.addEventListener("click", () => {
        shelterFileInput.click();
    });
    }

    if (shelterFileInput && shelterFileName) {
    shelterFileInput.addEventListener("change", () => {
        const file = shelterFileInput.files[0];

        if (file) {
        shelterFileName.textContent = file.name;
        } else {
        shelterFileName.textContent = "첨부파일";
        }
    });
  }
